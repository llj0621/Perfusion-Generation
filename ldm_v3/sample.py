"""Generate CTP sequences from first frame using trained LDM v3 (mask-conditioned).
...
Usage:
    python ldm_v3/sample.py --ldm_ckpt ldm_v3/checkpoints/ckpt_final.pt --subject 0
    python ldm_v3/sample.py --ldm_ckpt ldm_v3/checkpoints/ckpt_final.pt --subject 0 --mode custom --custom_hr 80
"""
import os
import sys
import json
...
def plot_comparison(gt_seq, gen_seq, time_frames, fname, save_path):
    ...
    fig.suptitle(f"LDM v3 Generation — {fname} (Window: 0-400 HU)", fontsize=11)
    plt.tight_layout()
...
def main():
    parser = argparse.ArgumentParser(description="LDM v3 inference")
    ...
    parser.add_argument("--out_dir", type=str, default="./ldm_v3/samples")
